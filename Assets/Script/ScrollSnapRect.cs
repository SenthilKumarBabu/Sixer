...
public class ScrollSnapRect : Singleton<ScrollSnapRect>, IBeginDragHandler, IEndDragHandler, IDragHandler {
	...
	public void Reset() {
		...
		_pageCount = 10 - CONTROLLER.currentMatchWickets;
	...
	void Update() 
	{
		//_container.anchoredPosition = new Vector2 (_container.anchoredPosition.x, -12f);
		// if moving to target position
		if (_lerp) {
			// prevent overshooting with values greater than 1
			float decelerate = Mathf.Min(decelerationRate * Time.fixedUnscaledDeltaTime, 1f);
		...
		for (int i = 0; i < _pageCount; i++) {
			if (i != _currentPage) {
				this.transform.GetChild(0).GetChild (i).localScale = new Vector3 (0.7f, 0.7f, 0.7f);
			} else {
				this.transform.GetChild(0).GetChild (i).localScale = new Vector3 (1f, 1f, 1f);
			}
		}
	}

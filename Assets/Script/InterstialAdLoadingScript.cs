...
public class InterstialAdLoadingScript : Singleton<InterstialAdLoadingScript>
{
...
#endif
        if (!CONTROLLER.isAdRemoved && CONTROLLER.CanShowAdtoNewUser_Inter == 1)
        {
            if ( AdIntegrate.instance.checkTheInternet() && AdIntegrate.instance.isInterstitialReadyToPlay() && CONTROLLER.launchInternetAdEvent)
			{
                if (buttonIndex ==6) // Server Ball over
				{
                    GameModel.instance.DisplayFullScreenAd();
					return;
				}
			...
			{
				//AdIntegrate.instance.RequestInterestialAd();
				ShowTapToContinue();
			}
        }
        else
    ...
    public void OnCompletedInterstialAd()
    {
        ...
        switch (CONTROLLER.gameMode)
        {
            case "superover": CallSuperOverFunction();
                break;
            case "superMatchesGamemode": CallSuperMatchesFunction();
                break;
    ...
    private void CallSuperMatchesFunction()
    {
        SuperCrusadesController.instance.GameOverButtonClickEvents(buttonIndex);
    }

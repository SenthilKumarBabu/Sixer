...
/*		if (GoogleAnalyticsBinder.instance != null)
	...
	protected IEnumerator Start ()
	{
		...
		//customGO.SendMessage ("RequestServerAd");
    ...
    public void ShowCrusadeGameModeUI()
    {
        Scoreboard.instance.Hide(true);
        BatsmanInfo.instance.HideMe();
        PreviewScreen.instance.Hide(true);
        SuperCrusadesController.instance.ShowSeasonUI();
    }
	...
	{
		if (!CONTROLLER.isAdRemoved && CONTROLLER.gameMode != "multiplayer" && CONTROLLER.gameMode != "superover")
		{
			if (AdIntegrate.instance != null && CONTROLLER.currentMatchBalls % CONTROLLER.serverConfig.IBO == 0 && CONTROLLER.currentMatchBalls != 0 && CONTROLLER.CanShowAdtoNewUser_Inter == 1)
			{
                if (AdIntegrate.instance.CurrentSceneIndex == 2 && CONTROLLER.CurrentPage == "ingame" && CONTROLLER.gameMode != "multiplayer")
                {
	...
	public void CheckForOverComplete ()
	{
		...
		if (CONTROLLER.gameMode != "superover")
		{
            ...
            else if(currentBall == CONTROLLER.totalBallInOver-1)
			{
				...
				if (!CONTROLLER.isAdRemoved && CONTROLLER.gameMode != "multiplayer" && CONTROLLER.gameMode != "superover")
				{
					if (AdIntegrate.instance != null && CONTROLLER.currentMatchBalls % CONTROLLER.serverConfig.IBO == 0 && CONTROLLER.currentMatchBalls != 0)
					{
						InterstialAdLoadingScript.instance.ShowMe(6); // Over completed - Interstitial Call - 6 0r 12 balls
					}
				}
			}
			else
...
public void enableBlocker ()
	...
	{
		string overStr;
		int overs = CONTROLLER.currentMatchBalls / 6;
		int  balls = CONTROLLER.currentMatchBalls % 6;
//		overStr = "Overs " + overs + "." + balls;
		if (CONTROLLER.gameMode != "multiplayer")
			overStr = overs + "." + balls;//overStr = "Overs " + overs + "." + balls;
		else
			overStr = overs + "." + balls;
		return overStr;
	}

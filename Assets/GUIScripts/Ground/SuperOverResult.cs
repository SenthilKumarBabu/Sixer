...
public class SuperOverResult : Singleton<SuperOverResult> 
{
	...
	{
		if (CONTROLLER.serverConfig.IGQ == 0 && index == 0)
			GoToHome();
		else if (CONTROLLER.serverConfig.IGN == 0 && index == 1)
			GoNextLevel();
		else if (CONTROLLER.serverConfig.IGR == 0 && index == 2)
			ReplayThisLevel();
		else if (CONTROLLER.LevelId % 2 == 0 && (index == 1 || index == 2))
		{
			if (index == 1)
				GoNextLevel();
			else
	...
	public void ShareChallenge ()
	{
		...
		string body = CONTROLLER.UserName + " completed challenge no " + (1 + CONTROLLER.LevelId) + " in Super Over.\nDownload "+CONTROLLER.AppName +" now " + CONTROLLER.AppLink;
